...
impl<'de> de::Deserializer<'de> for Deserializer {
    ...
    {
        match self.term {
            Term::Atom(v) => match v.name.as_str() {
                "true" => visitor.visit_bool(true),
                "false" => visitor.visit_bool(false),
                _ => visitor.visit_string(v.name),
            },
            Term::FixInteger(v) => visitor.visit_i32(v.value),
            Term::BigInteger(v) => visitor.visit_i64(
                v.to_i64().ok_or(Error::IntegerConvertError)?,
            ),
            ...
            Term::Binary(v) => match String::from_utf8(v.bytes.to_vec()) {
    ...
    {
        match self.term {
            Term::Atom(atom) => visitor.visit_string(atom.name),
            Term::Binary(binary) => visitor.visit_bytes(&binary.bytes),
