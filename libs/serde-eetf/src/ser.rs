...
impl<'a> ser::Serializer for &'a Serializer {
    ...
    fn serialize_bool(self, v: bool) -> Result<Term> {
        ...
        Ok(Term::Atom(eetf::Atom::from(match v {
            true => "true",
            false => "false",
        })))
    }
    ...
    fn serialize_map(self, len: Option<usize>) -> Result<Self::SerializeMap> {
        ...
        Ok(MapSerializer { items: vec, last_key: None })
    }
...
impl ser::SerializeTupleVariant for NamedSequenceSerializer {
    type Ok = Term;
    type Error = Error;
...
impl ser::SerializeMap for MapSerializer {
    ...
    {
        let serializer = Serializer {};
        let term = key.serialize(&serializer)?;
        self.last_key = Some(term);
            ...
            T: ?Sized + Serialize,
    {
        let serializer = Serializer {};
        let term = value.serialize(&serializer)?;
        self.items.push((self.last_key.clone().expect("No key for map"), term));
...
impl ser::SerializeStructVariant for NamedMapSerializer {
    type Ok = Term;
    type Error = Error;
